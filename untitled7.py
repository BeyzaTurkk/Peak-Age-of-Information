...
def plot_correlated_resets_vs_burst(
    ...
    plt.ylim(bottom=0)
    plt.grid(True)
    plt.title(f"Random Resets on Correlated Channel (M={M}, ε={epsilon})")
...
def plot_correlated_resets_vs_burst(
    ...
    # Gilbert–Elliott with p_good=1, p_bad=0.
    #  ε = q/(q+r) Burst B≈1/r => r=1/B, q = ε r / (1-ε).
...
# 2) IID channel: Peak AoI (normalized) vs ε (PER), curves for different K
def plot_iid_resets_vs_epsilon(
    N=100, M=4,
    eps_list=np.linspace(0.0, 0.4, 9),
    K_list=(0, 2, 5, 10),
